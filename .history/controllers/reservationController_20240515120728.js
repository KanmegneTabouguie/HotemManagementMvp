...
const reservationController = {
    createReservation: async (req, res) => {
        try {
            ...
            console.log('Room details:', room); // Add debug logging
            ...
            console.log('Price per night:', pricePerNight); // Add debug logging

            const checkInDate = new Date(check_in_date);
            const checkOutDate = new Date(check_out_date);
            const diffTime = Math.abs(checkOutDate - checkInDate);
            const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
            console.log('Difference in days:', diffDays); // Add debug logging

            const total_amount = diffDays * pricePerNight;
            console.log('Total amount:', total_amount); // Add debug logging
