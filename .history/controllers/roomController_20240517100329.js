...
exports.listAvailableRooms = async (req, res) => {
    try {
        ...
        if (!rooms) {
            ...
            cache.put('rooms', rooms, CACHE_DURATION); // Cache the rooms
        }
...
exports.createRoom = async (req, res) => {
    ...
    try {
        ...
        // Invalidate cache
        cache.clear(); // Clear all caches when a new room is created
...
exports.getRoomById = async (req, res) => {
    try {
        ...
        // Check cache
        let room = cache.get(`room_${roomId}`);
        if (!room) {
            room = await roomService.getRoomById(roomId);
            if (!room) {
                return res.status(404).json({ message: 'Room not found' });
            }
            cache.put(`room_${roomId}`, room, CACHE_DURATION); // Cache the room
        }
...
exports.updateRoom = async (req, res) => {
    try {
        ...
        // Invalidate cache
        cache.clear(); // Clear all caches when a room is updated
...
exports.partialUpdateRoom = async (req, res) => {
    try {
        ...
        // Invalidate cache
        cache.clear(); // Clear all caches when a room is partially updated
...
exports.deleteRoom = async (req, res) => {
    try {
        ...
        // Invalidate cache
        cache.clear(); // Clear all caches when a room is deleted
